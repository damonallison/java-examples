...
public class ClassTests
{
	 ...
	 *
	 * By not providing multiple inheritance, java prevents problems that arise from
	 * fields that exist in multiple superclasses. If a field "value" exists in multiple
	 * superclasses, which field should the subclass use?
	 *
	 * Interface inheritance has a similar problem with default methods. If the same default
	 * method is defined on multiple interfaces (with different implementations), which
	 * method will the subclass use? Can we tell java which one should be used?
	 *
	 * In this example, the class MultipleInheritance inherits from two interfaces
	 * with a {@code getValue()} member. This is a compile error. In order to fix the error,
	 * you must implement the "class wins" rule and override the ambiguous member in your
	...
	/**
	 * The term "polymorphism" is the ability for a single object to have many different forms, or
	 * be represented as different types.
	 *
	 * All objects are polymorphic - they can be represented as their type or as {@link Object}.
	 * Classes which implement interfaces or superclasses can be represented as those types in
	 * addition to {@link Object}.
	 *
	 */
	@Test
	public void testPolymorphism() {
		// Shows polymorphism. Here, bike is represented as an IBike, Bike, and MountainBike.
		// In all cases, the methods that are invoked are from the actual type, not the type that
		// the object is casted to at the moment.
		IBike bike = MountainBikeBuilder.newBuilder().setMaxElevation(100).setGear(1).setSpeed(10).setWheelCount(2).build();
		Bike asBike = (Bike)bike;
		MountainBike asMountainBike = (MountainBike)bike;

		// These comparisons are actually pointless - they'll always refer to the same methods.
		// This shows the concept of polymorphism by showing the same object being used as
		// different types.
